...
class HanteiController:
    ...
    def judge(self, frame: np.ndarray) -> str:
        ...
        if self.class_name is not None:
            self.display_str = f"class: {self.class_name}"

        cv2.putText(
            src, self.display_str, (30, 30), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 1,
        )
        ...
        # 白色領域のノイズを除去する
        hsv_mask = cv2.erode(hsv_mask, self.kernel)  # 収縮処理

        hsv_mask = cv2.dilate(hsv_mask, self.kernel)  # 膨張処理
